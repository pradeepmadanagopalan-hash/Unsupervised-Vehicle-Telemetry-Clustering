'''
0. README
Notebook: Wasserstein_Distance_Weight_Spectra
Inputs
...
•	The notebook then passes the weight wasserstein distance to unsupervised k-medoids and segregates each vin into one of 2 clusters.
•	Sales group information is added back and the final output of this notebook is a dataframe which has 3 columns vin - corresponding sales group - corresponding weight cluster assigned by k-medoids.
Other remarks
•	Note that the unsupervised clustering algorithm only gives you cluster information as a numeric integeer e.g. 0 or 1. Assigning & interpreting the clusters as High & Low should be done manually by looking at the visualization in section 10 of this notebook.
...
# 3. Display rows to be removed, repeated step just to be sure
...
# -------------------------------
df_vin_group = df_weight_all.select('vin', 'sales_group').distinct().toPandas()

# Align the VIN order with df_truck_distributions
...
# -------------------------------
# Plot average weight distribution per cluster
# -------------------------------
...
plt.xlabel("Gross CombinationW Weight (kg)")
plt.ylabel("Normalized Mileage")
plt.title("Average weight distribution per Wasserstein cluster")
plt.legend()
plt.grid(True)
plt.show()
...
'''
# -------------------------------
# 2. Define consistent cluster colors for 2 clusters
# -------------------------------
cluster_colors = {
    0: "#ff7f0e",   # orange (high weight)
    1: "#1f77b4"    # blue (low weight)
}
...
cluster_colors = {
    0: "#1f77b4",   # blue (high weight)
    1: "#ff7f0e"    # orange (low weight)
}
...
for idx, sg in enumerate(sales_groups, 1):
    ...
    ax.set_title(f"Sales Group {sg}: Weight Cluster Distribution", fontsize=14)
...
df_cluster_spark.write.mode("overwrite").saveAsTable("pt_configuration_thesis.alltrucks_v1.weight_cluster_results")
